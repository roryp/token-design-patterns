...
public class PatternCatalog {
    ...
    private static PatternDefinition caching() {
        return new PatternDefinition(
                ...
                "The graph replays provider-reported HIT or MISS, then always passes through Terra for a fresh answer. A return-to-cache path lights only for reported writes. These are provider internals, not extra Agentic steps.",
                ...
                List.of(
                        n("input", "User request", "new question", "user", 7, 50),
                        n("cache", "Prompt cache", "same prefix + same deployment", "cache", 30, 50),
                        n("hit", "HIT", "reuse cached input processing", "cache-hit", 52, 22),
